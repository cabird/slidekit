...
function openDiffModal() {
  ...
  title2.style.cssText = "font-size: 14px; font-weight: 600;";
  title2.textContent = "Slide Diff";
...
function startEdit(elementId, propKey, currentValue, propDiv, slideIndex) {
  ...
  label2.textContent = propKey.endsWith(".gap") ? "gap=" : `${propKey}: `;
  label2.style.color = "#999";
  propDiv.appendChild(label2);
...
function startGapTokenEdit(elementId, propKey, currentValue, propDiv, slideIndex) {
  ...
  propDiv.textContent = "";
  const label2 = document.createElement("span");
  label2.textContent = isConstraintGap ? "gap=" : `${propKey}: `;
  label2.style.color = "#999";
  propDiv.appendChild(label2);
  propDiv.appendChild(select);
  propDiv.style.textDecoration = "none";
...
function createDropdownPanel(elementId, constraintType, slideIndex, sceneElements, validRefs, invalidRefs) {
  ...
  title2.style.cssText = "font-weight: 600; margin-bottom: 8px; color: #1a1a2e;";
  title2.textContent = `Pick reference for ${constraintType}`;
...
function createMenuItem(label2, opts = {}) {
...
function createSectionHeader(label2) {
  const header = document.createElement("div");
  header.style.cssText = `
...
function showGroupNameInput(popover, opt, elementId, slideIndex) {
  popover.innerHTML = "";
  const title2 = document.createElement("div");
  title2.style.cssText = "padding: 6px 12px 4px; font-weight: 600; color: #1a1a2e; font-size: 11px;";
  title2.textContent = opt.label.replace("...", "");
...
function createVisibilitySection() {
  ...
  const title2 = document.createElement("span");
  title2.textContent = "VISIBILITY";
  title2.style.fontSize = "11px";
  title2.style.fontWeight = "600";
  title2.style.letterSpacing = "0.5px";
  title2.style.color = "#777";
  header.appendChild(title2);
  ...
  for (const opt of VISIBILITY_OPTIONS) {
    ...
    label2.appendChild(document.createTextNode(opt.label));
    grid2.appendChild(label2);
  }
...
function refreshElementList() {
  ...
  header.appendChild(chevron);
  const title2 = document.createElement("span");
  title2.textContent = "ELEMENTS & CONSTRAINTS";
  title2.style.cssText = "font-size: 11px; font-weight: 600; letter-spacing: 0.5px; color: #777;";
  header.appendChild(title2);
  section.appendChild(header);
  ...
  for (const layerName of LAYER_ORDER) {
    ...
    for (const item of items) {
      ...
      label2.style.color = s.selectedElementId === item.id ? "#4a9eff" : "#333";
      label2.style.fontWeight = s.selectedElementId === item.id ? "600" : "normal";
...
function createSection(title2, content, collapsed = false) {
  const section = document.createElement("div");
  section.setAttribute("data-sk-inspector-section", title2);
  section.style.borderBottom = "1px solid #e8e8e8";
...
function isConnectorEndpointPair(a, b) {
  if (a.type === "connector") {
    ...
    const fromId = cr?.from.elementId ?? props?.fromId;
    const toId = cr?.to.elementId ?? props?.toId;
...
function buildConnectorSVG(conn, elementMarkerId) {
  const { from, to, style, path, label: label2 } = conn;
  ...
  if (connType === "curved" && path.cx1 !== void 0) {
  ...
  if (label2) {
    const anchorX = label2.x - minX;
    const anchorY = label2.y - minY;
    const { angle, text, style: ls } = label2;
...
function finalize({
  ...
  const portGroupKey = (direction, elementId, edge) => `${direction}\0${elementId}\0${edge}`;
  ...
  for (const id of sortedOrder) {
    ...
    if (fromEdge !== "center") {
      ...
      let group2 = portGroups.get(key);
      if (!group2) {
        group2 = { direction: "from", elementId: fromId, edge, entries: [] };
        portGroups.set(key, group2);
      }
      group2.entries.push({
        idx,
    ...
    if (toEdge !== "center") {
      ...
      let group2 = portGroups.get(key);
      if (!group2) {
        group2 = { direction: "to", elementId: toId, edge, entries: [] };
        portGroups.set(key, group2);
      }
      group2.entries.push({
        idx,
  ...
  for (const group2 of portGroups.values()) {
    const { direction, elementId, edge, entries } = group2;
    ...
    const spacing = totalSpread > usable ? usable / (entries.length - 1) : PORT_SPACING;
  ...
  for (const info of connectorInfos) {
    ...
    if (sceneElements[id]) {
      ...
      const rawColor = el2.props.color ?? "#ffffff";
      const resolvedColor = resolveCssColor(rawColor);
      const thicknessResolved = el2.props.thickness ?? 2;
      const arrowResolved = el2.props.arrow ?? "end";
      const dashResolved = el2.props.dash ?? null;
      const opacityResolved = el2.props.opacity ?? 1;
      const resolvedPath = {
        x1: fromPt.x,
        y1: fromPt.y,
        x2: toPt.x,
      ...
      if (connType === "elbow" || connType === "orthogonal") {
        const cornerRadius = el2.props.cornerRadius;
        if (cornerRadius !== void 0 && cornerRadius > 0) {
          resolvedPath.cornerRadius = cornerRadius;
        }
      }
      let resolvedLabel;
      const labelText = el2.props.label;
      if (labelText) {
        const labelStyle = el2.props.labelStyle ?? {};
        const fontSize = labelStyle.size ?? labelStyle.fontSize ?? 14;
        const fontColor = labelStyle.color ?? "#999999";
        const fontFamily = labelStyle.font ?? labelStyle.fontFamily ?? "Inter";
        const fontWeight = labelStyle.weight ?? labelStyle.fontWeight ?? 400;
        const labelPosition = el2.props.labelPosition ?? 0.5;
        const labelOffset = el2.props.labelOffset ?? {};
  ...
  for (const layer of Object.keys(layerElements)) {
    ...
    for (let i = 0; i < ids.length; i++) {
      for (let j = i + 1; j < ids.length; j++) {
        ...
        let boundsA = absoluteBoundsOf2(idA);
        let boundsB = absoluteBoundsOf2(idB);
...
var FONT = "Inter";
function box(id, label2, x, y, w = 160, h = 80, bg = C.glass) {
  return el(
    `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:${bg};border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">${label2}</div>`,
    { id, x, y, w, h }
  );
}
function title(text, id, extra = {}) {
  return el(
    `<h2 style="font-family:${FONT};font-size:48px;font-weight:700;color:${C.text};line-height:1.2;">${text}</h2>`,
    { id, w: 1680, h: 70, ...extra }
  );
}
function label(text, id, extra = {}) {
  return el(
    `<span style="font-family:${FONT};font-size:18px;color:${C.textSec};">${text}</span>`,
    { id, ...extra }
  );
}
var PLACEHOLDER_SVG = `data:image/svg+xml,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300"><rect fill="#334" width="400" height="300"/><text x="200" y="150" text-anchor="middle" fill="white" font-size="20">Image</text></svg>')}`;
async function run() {
  await init({
    slide: { w: 1920, h: 1080 },
    safeZone: { left: 120, right: 120, top: 90, bottom: 90 },
  ...
  const slides = [
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(0,212,255,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">tl</div>`,
          { id: "s4-tl", x: 560, y: 310, w: 140, h: 70, anchor: "tl" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(124,92,191,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">tc</div>`,
          { id: "s4-tc", x: 960, y: 310, w: 140, h: 70, anchor: "tc" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(255,107,157,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">tr</div>`,
          { id: "s4-tr", x: 1360, y: 310, w: 140, h: 70, anchor: "tr" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(0,255,136,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">cl</div>`,
          { id: "s4-cl", x: 560, y: 540, w: 140, h: 70, anchor: "cl" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(255,165,0,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">cc</div>`,
          { id: "s4-cc", x: 960, y: 540, w: 140, h: 70, anchor: "cc" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(255,0,0,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">cr</div>`,
          { id: "s4-cr", x: 1360, y: 540, w: 140, h: 70, anchor: "cr" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(200,200,0,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">bl</div>`,
          { id: "s4-bl", x: 560, y: 770, w: 140, h: 70, anchor: "bl" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(0,100,200,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">bc</div>`,
          { id: "s4-bc", x: 960, y: 770, w: 140, h: 70, anchor: "bc" }
        ),
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(180,0,180,0.25);border:1px solid ${C.glassBr};border-radius:8px;font-family:${FONT};font-size:16px;color:${C.text};">br</div>`,
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        label("vstack align: center", "s5-lbl2", { x: 1e3, y: 190, w: 300, h: 30 }),
        vstack([
          el(
            `<div style="background:rgba(0,255,136,0.15);border:1px solid ${C.glassBr};border-radius:8px;width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-family:${FONT};font-size:14px;color:${C.text};">Wide: 500px</div>`,
            { id: "s5-vs-a", w: 500, h: 50 }
          ),
          el(
            `<div style="background:rgba(255,165,0,0.15);border:1px solid ${C.glassBr};border-radius:8px;width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-family:${FONT};font-size:14px;color:${C.text};">Medium: 300px</div>`,
            { id: "s5-vs-b", w: 300, h: 50 }
          ),
          el(
            `<div style="background:rgba(200,200,0,0.15);border:1px solid ${C.glassBr};border-radius:8px;width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-family:${FONT};font-size:14px;color:${C.text};">Narrow: 150px</div>`,
            { id: "s5-vs-c", w: 150, h: 50 }
          )
        ], { id: "s5-vstack", x: 1e3, y: 230, gap: 12, align: "center" })
      ]
    },
    // ================================================================
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        panel([
          el(`<span style="font-family:${FONT};font-size:14px;color:${C.accent1};">Outer Panel</span>`, { id: "s9-outer-lbl", w: "fill", h: 24 }),
          panel([
            el(`<span style="font-family:${FONT};font-size:14px;color:${C.accent2};">Middle Panel</span>`, { id: "s9-mid-lbl", w: "fill", h: 24 }),
            panel([
              el(`<span style="font-family:${FONT};font-size:14px;color:${C.accent3};">Inner Panel</span>`, { id: "s9-inner-lbl", w: "fill", h: 24 })
        ...
        panel([
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 1</span>`, { id: "s9-item1", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 2</span>`, { id: "s9-item2", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 3</span>`, { id: "s9-item3", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 4</span>`, { id: "s9-item4", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 5</span>`, { id: "s9-item5", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 6</span>`, { id: "s9-item6", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 7</span>`, { id: "s9-item7", w: "fill", h: 22 }),
          el(`<span style="font-family:${FONT};font-size:13px;color:${C.text};">Item 8</span>`, { id: "s9-item8", w: "fill", h: 22 })
        ], { id: "s9-many", x: 700, y: 200, w: 300, fill: C.glass, radius: 10, border: `1px solid ${C.glassBr}`, padding: 12, gap: "sm" }),
        // Tiny padding panel
        panel([
          el(`<span style="font-family:${FONT};font-size:12px;color:${C.text};">Tiny pad (2px)</span>`, { id: "s9-tiny-lbl", w: "fill", h: 20 })
        ], { id: "s9-tiny", x: 1100, y: 200, w: 200, fill: C.glass, radius: 4, border: `1px solid ${C.glassBr}`, padding: 2 }),
        // Huge padding panel
        panel([
          el(`<span style="font-family:${FONT};font-size:14px;color:${C.text};">Huge pad (60px)</span>`, { id: "s9-huge-lbl", w: "fill", h: 24 })
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        el(
          `<div style="width:100%;height:100%;border:2px dashed rgba(255,255,255,0.15);border-radius:4px;"></div>`,
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        // Very small text
        el(
          `<span style="font-size:10px;font-family:${FONT};color:${C.text};">Very small text (10px) \u2014 can you even read this?</span>`,
          { id: "s11-tiny", x: 150, y: 200, w: 400, h: 20 }
        ),
        // Very large text
        el(
          `<span style="font-size:100px;font-family:${FONT};font-weight:800;color:${C.accent1};">BIG</span>`,
          { id: "s11-huge", x: 150, y: 240, w: 600, h: 130 }
        ),
        // Very long single line
        el(
          `<span style="font-size:16px;font-family:${FONT};color:${C.textSec};white-space:nowrap;">This is an extremely long single line of text that goes on and on and on without wrapping to test how SlideKit handles very long unwrapped text content that extends way beyond what you would normally expect in a slide element</span>`,
          { id: "s11-long", x: 150, y: 400, w: 1700, h: 30, style: { overflow: "hidden" } }
        ),
        // Tight line-height
        el(
          `<span style="font-size:20px;font-family:${FONT};color:${C.text};line-height:1.0;">Tight line-height (1.0)<br>Second line<br>Third line</span>`,
          { id: "s11-tight", x: 150, y: 480, w: 400, h: 80 }
        ),
        // Wide line-height
        el(
          `<span style="font-size:20px;font-family:${FONT};color:${C.text};line-height:2.5;">Wide line-height (2.5)<br>Second line<br>Third line</span>`,
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        cardGrid([
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent1};">Card 1</span>`, { id: "s12-c1-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">Short content</span>`, { id: "s12-c1-body", w: "fill", h: 20 })
          ], { id: "s12-c1", w: 480, fill: C.glass, radius: 12, border: `1px solid ${C.glassBr}`, padding: 16, gap: "sm" }),
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent2};">Card 2</span>`, { id: "s12-c2-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">This card has significantly more content to test how the grid handles uneven heights across columns.</span>`, { id: "s12-c2-body", w: "fill", h: 60 })
          ], { id: "s12-c2", w: 480, fill: C.glass, radius: 12, border: `1px solid ${C.glassBr}`, padding: 16, gap: "sm" }),
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent3};">Card 3</span>`, { id: "s12-c3-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">Medium text</span>`, { id: "s12-c3-body", w: "fill", h: 30 })
          ], { id: "s12-c3", w: 480, fill: C.glass, radius: 12, border: `1px solid ${C.glassBr}`, padding: 16, gap: "sm" }),
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent4};">Card 4</span>`, { id: "s12-c4-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">Content</span>`, { id: "s12-c4-body", w: "fill", h: 20 })
          ], { id: "s12-c4", w: 480, fill: C.glass, radius: 12, border: `1px solid ${C.glassBr}`, padding: 16, gap: "sm" }),
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent1};">Card 5</span>`, { id: "s12-c5-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">Another card here</span>`, { id: "s12-c5-body", w: "fill", h: 20 })
          ], { id: "s12-c5", w: 480, fill: C.glass, radius: 12, border: `1px solid ${C.glassBr}`, padding: 16, gap: "sm" }),
          panel([
            el(`<span style="font-family:${FONT};font-size:16px;font-weight:600;color:${C.accent2};">Card 6</span>`, { id: "s12-c6-title", w: "fill", h: 24 }),
            el(`<span style="font-family:${FONT};font-size:13px;color:${C.textSec};">Last card in the 3-column grid</span>`, { id: "s12-c6-body", w: "fill", h: 20 })
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        el(
          `<div style="width:100%;height:100%;background:linear-gradient(135deg, rgba(0,212,255,0.15), rgba(124,92,191,0.15));border-radius:16px;"></div>`,
        ...
        // Overlay layer text
        el(
          `<div style="font-family:${FONT};font-size:36px;font-weight:700;color:${C.accent1};text-align:center;background:rgba(0,0,0,0.6);padding:16px 32px;border-radius:12px;">OVERLAY</div>`,
          { id: "s14-overlay", x: 500, y: 430, w: 300, h: 80, layer: "overlay" }
        ),
        // Second independent set
        el(
          `<div style="width:100%;height:100%;background:rgba(255,165,0,0.1);border-radius:12px;"></div>`,
          { id: "s14-bg2", x: 1200, y: 250, w: 500, h: 500, layer: "bg" }
        ),
        el(
          `<span style="font-family:${FONT};font-size:20px;color:${C.text};">Layered content on bg</span>`,
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        // Default bias (0.5)
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(0,255,136,0.2);border:2px dashed ${C.accent4};border-radius:8px;font-family:${FONT};font-size:14px;color:${C.text};">between default</div>`,
        ...
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(124,92,191,0.2);border:2px dashed ${C.accent2};border-radius:8px;font-family:${FONT};font-size:14px;color:${C.text};">between bias:0.75</div>`,
        ...
        el(
          `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:rgba(200,200,0,0.15);border:1px dashed ${C.textSec};border-radius:4px;font-family:${FONT};font-size:12px;color:${C.text};">Tight fit</div>`,
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        // Near-zero size
        label("1\xD71 pixel \u2193", "s16-lbl2", { x: 500, y: 200, w: 200, h: 24 }),
        ...
        el(
          `<span style="font-family:${FONT};font-size:16px;color:${C.accent4};">Auto sized text</span>`,
          { id: "s16-auto", x: 700, y: 240 }
        ),
        // Empty group
        label("Empty group \u2193", "s16-lbl4", { x: 1e3, y: 200, w: 200, h: 24 }),
        group([], { id: "s16-empty-group", x: 1e3, y: 240 }),
        // Visible marker after empty group to confirm rendering continues
        el(
          `<div style="background:${C.accent1};border-radius:50%;width:100%;height:100%;"></div>`,
          { id: "s16-after-empty", x: 1e3, y: 280, w: 16, h: 16 }
        )
      ]
    },
    // ================================================================
    ...
    // ================================================================
    {
      ...
      elements: [
        ...
        el(`<div style="width:100%;height:100%;background:${C.accent1};opacity:0.7;border-radius:8px;display:flex;align-items:center;justify-content:center;font:600 16px ${FONT};color:#000;">z=1 (behind)</div>`, {
          id: "s21-z1",
          x: rightOf("s21-img-contain", { gap: 40 }),
          y: below("s21-text-br", { gap: 20 }),
          w: 200,
          h: 150,
          z: 1
        ...
        el(`<div style="width:100%;height:100%;background:${C.accent3};opacity:0.8;border-radius:8px;display:flex;align-items:center;justify-content:center;font:600 16px ${FONT};color:#000;">z=3 (front)</div>`, {
          id: "s21-z3",
          x: rightOf("s21-img-contain", { gap: 70 }),
        ...
        el(`<div style="width:100%;height:100%;background:${C.accent4};opacity:0.8;border-radius:8px;display:flex;align-items:center;justify-content:center;font:600 16px ${FONT};color:#000;">z=2 (middle)</div>`, {
          id: "s21-z2",
          x: rightOf("s21-img-contain", { gap: 55 }),
...
//# sourceMappingURL=stress_test_bundle.js.map
